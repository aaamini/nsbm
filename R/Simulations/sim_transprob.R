...
res <- res %>%
  mutate(method = factor(method
                         , levels = c(sort(mtd_names[1:3]), sort(mtd_names[4:6]))
                         , labels = c(sort(mtd_names[1:3]), sort(mtd_names[4:6]))))
...
p_z <- mean_res %>%
  ...
  scale_fill_manual(values = scales::hue_pal()(7)[c(2:7)]) +
  scale_color_manual(values = scales::hue_pal()(7)[c(2:7)])
